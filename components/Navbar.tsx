      ...
      <nav className="fixed top-0 w-full h-14 md:h-16 bg-slate-950/80 backdrop-blur-xl border-b border-white/10 px-4 md:px-6 flex items-center justify-between z-[60]">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex items-center gap-1 font-black text-[10px] tracking-[0.2em]">
            {menuItems.map((item) => ( <Link key={item.href} href={item.href} className={`px-4 py-2 rounded-lg border transition-all ${pathname === item.href ? 'bg-blue-600/10 text-blue-400 border-blue-500/20' : 'text-slate-400 border-transparent hover:text-slate-200'}`}>{item.name}</Link> ))}
          </div>
        </div>

        <div className="flex items-center gap-3 relative" ref={profileRef}>
          <button onClick={() => window.dispatchEvent(new CustomEvent('open-cart'))} className="relative p-2.5 text-slate-400 hover:text-white bg-white/5 rounded-full border border-white/5"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] flex items-center justify-center bg-blue-600 text-white text-[8px] font-black rounded-full border-2 border-slate-950 px-1">{cartCount}</span>}</button>
