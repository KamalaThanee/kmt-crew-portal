...
export default function ApprovalsPage() {
  ...
  if (loading) return <div className="p-10 text-center text-blue-500 font-black animate-pulse">LOADING PENDING REQUESTS...</div>

  return (
    <div className="p-6 max-w-4xl mx-auto pb-32 pt-20">
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 gap-4">
        {requests.length === 0 && (
          <div className="py-20 text-center bg-slate-900/50 rounded-[32px] border border-dashed border-white/10">
            <Clock className="mx-auto mb-4 text-slate-700" size={48}/>
            <p className="text-slate-500 font-bold uppercase text-xs tracking-widest">No pending requests at the moment</p>
          </div>
        )}
        {requests.map(req => (
          <div key={req.id} className="bg-slate-900 border border-white/5 p-6 rounded-[32px] space-y-6">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-white font-black uppercase text-sm leading-tight">{req.crew_name}</h3>
              ...
              <div className="space-y-2">
                {req.items?.map((item: any, idx: number) => (
                  <div key={idx} className="flex justify-between text-xs font-bold">
                    ...
                    <span className="text-blue-400 uppercase">{item.color} | SIZE: {item.size}</span>
                  </div>
                ))}
              </div>
            </div>
