    ...
    <div className="min-h-screen bg-black text-white font-sans pb-32 pt-24 px-4 md:px-12 uppercase font-bold text-[10px]">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex-1 bg-zinc-900/50 border border-white/5 rounded-[48px] p-8 md:p-12 shadow-inner min-h-[70vh]">
            
            {/* 📦 TAB: INVENTORY MASTER (RESTORED) */}
            {activeTab === 'inventory' && (
              <div className="animate-in fade-in space-y-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                   <h2 className="text-2xl font-black italic text-white">Inventory Management</h2>
                   <button onClick={() => { setEditingItem({ item_name: '', category: 'Other', quantity: 0, threshold: 1, item_id_code: generateNextCode('Other') }); setIsItemModalOpen(true); }} className="w-full md:w-auto px-8 py-4 bg-orange-600 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 shadow-lg hover:bg-orange-500 transition-all"><Plus size={18}/> Add New Item</button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-black/40 p-5 rounded-3xl border border-white/5">
                   ...
                   <select className="bg-zinc-900 border border-white/10 p-4 rounded-2xl outline-none text-xs" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}><option value="all">All Stock Status</option><option value="low">Low Stock Only</option></select>
                </div>
                ...
                <div className="space-y-4">
                  {Object.entries(filteredInventoryGrouped).map(([category, items]) => {
                    ...
                    return (
                      <div key={category} className={`border rounded-[32px] overflow-hidden transition-all ${isExpanded ? 'border-orange-500/50 bg-black/20' : 'border-white/5 bg-zinc-900/30'}`}>
                        <button onClick={() => setExpandedCats(isExpanded ? expandedCats.filter(c => c !== category) : [...expandedCats, category])} className="w-full p-6 flex justify-between items-center hover:bg-white/5">
                          <div className="flex items-center gap-4 text-orange-500"><Box size={24}/><h3 className="text-sm font-black tracking-widest text-white uppercase">{category} <span className="ml-2 text-zinc-600">({items.length})</span></h3></div>
                        ...
                        {isExpanded && (
                          <div className="overflow-x-auto border-t border-white/5 p-4">
                            <table className="w-full text-left text-[11px] font-black uppercase whitespace-nowrap border-separate border-spacing-y-2">
                              <thead className="text-zinc-500 bg-black/40"><tr className="rounded-xl overflow-hidden"><th className="p-4">Code</th><th className="p-4">Item Name</th><th className="p-4">Spec</th><th className="p-4 text-right">Qty</th><th className="p-4 text-center">Edit</th></tr></thead>
                              <tbody className="text-zinc-300">
                                {items.map(item => (
                                  <tr key={item.id} className="bg-white/5 hover:bg-orange-600/10 transition-all">
                                    ...
                                    <td className="p-4 text-center rounded-r-xl"><button onClick={() => { setEditingItem(item); setIsItemModalOpen(true); }} className="p-2 bg-white/5 rounded-lg text-orange-400 hover:bg-orange-600 hover:text-white transition-all"><Edit size={16}/></button></td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
            ...
            {/* 🎯 TAB: CREW MASTER (RESTORED) */}
            {activeTab === 'crews' && (
              <div className="animate-in fade-in space-y-8">
                <h2 className="text-2xl font-black italic text-white mb-4 uppercase">Crew Management</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                   {[
                     ...
                     <button key={tile.id} onClick={() => setFilterMode(tile.id)} className={`bg-black/40 p-5 rounded-3xl border-t-4 ${tile.color} shadow-lg flex flex-col items-center justify-center transition-all ${filterMode === tile.id ? 'bg-zinc-800 border-white/20' : 'opacity-70'}`}>
                ...
                <div className="space-y-4">
                  {enhancedCrews.map(crew => (
                    <div key={crew.id} className="flex justify-between items-center bg-black/40 p-6 rounded-[32px] border border-white/5 group hover:border-orange-500/50 transition-all cursor-pointer" onClick={() => { setEditingCrew(crew); setIsEditCrewOpen(true); }}>
                      ...
                      <div><p className="font-bold text-lg text-white group-hover:text-orange-500 transition-colors">{crew.full_name} {crew.certData.expired > 0 && <span className="bg-red-500 text-white px-3 py-1 rounded-full text-[9px] animate-pulse ml-2">EXP {crew.certData.expired}</span>}</p><p className="text-xs text-zinc-500 uppercase tracking-widest mt-1">{crew.position}</p></div></div>
              ...
              <div className="animate-in fade-in grid grid-cols-1 md:grid-cols-2 gap-10">
                {['suit', 'boot'].map(type => (
                  <div key={type} className="p-10 bg-black/40 rounded-[48px] border border-white/5 text-center space-y-6">
                    ...
                    <div className="w-full h-72 bg-zinc-950 rounded-[40px] border border-orange-500/10 flex items-center justify-center overflow-hidden shadow-inner">{sizeCharts[type as 'suit' | 'boot'] ? <img src={sizeCharts[type as 'suit' | 'boot']} className="max-w-full max-h-full object-contain" /> : <p className="text-zinc-800 italic uppercase font-black text-xl">No Image</p>}</div>
                    <label className="flex items-center justify-center w-full py-5 bg-orange-600 rounded-3xl cursor-pointer hover:bg-orange-500 transition-all text-white font-black text-xs uppercase shadow-xl"><Upload size={20} className="mr-3"/> Update Chart Image<input type="file" className="hidden" accept="image/*" onChange={(e) => e.target.files?.[0] && handleUpload(type as 'suit' | 'boot', e.target.files[0])} /></label>
                  </div>
                ))}
              </div>
            )}
      ...
      {isItemModalOpen && editingItem && (
        <div className="fixed inset-0 z-[1000] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl animate-in zoom-in duration-300">
          <div className="bg-zinc-900 border border-orange-500/20 rounded-[48px] w-full max-w-xl p-12 space-y-8 shadow-[0_0_100px_rgba(249,115,22,0.1)] max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="flex justify-between items-center border-b border-white/5 pb-6"><h2 className="text-2xl font-black italic uppercase text-orange-500 tracking-tighter">{editingItem.id ? 'Edit' : 'Add New'} Inventory</h2><button onClick={() => setIsItemModalOpen(false)} className="p-3 bg-white/5 rounded-full hover:bg-red-500 transition-all"><X size={24}/></button></div>
            <div className="grid grid-cols-2 gap-6">
              <div className="col-span-2 space-y-2"><label className="text-orange-500 font-black ml-1">CATEGORY *</label><select className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold text-sm" value={editingItem.category} onChange={e => {const newCat = e.target.value; setEditingItem({...editingItem, category: newCat, item_id_code: editingItem.id ? editingItem.item_id_code : generateNextCode(newCat)})}}>{allCategories.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
              <div className="col-span-2 space-y-2"><label className="font-black ml-1">ITEM NAME *</label><input className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold text-sm" value={editingItem.item_name} onChange={e => setEditingItem({...editingItem, item_name: e.target.value})}/></div>
              ...
              <div className="space-y-2"><label className="font-black ml-1">UNIT</label><input className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold text-sm" value={editingItem.unit} onChange={e => setEditingItem({...editingItem, unit: e.target.value})}/></div>
              <div className="space-y-2"><label className="font-black ml-1">COLOR</label><input className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold text-sm" value={editingItem.color} onChange={e => setEditingItem({...editingItem, color: e.target.value})}/></div>
              <div className="space-y-2"><label className="font-black ml-1">SIZE</label><input className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold text-sm" value={editingItem.size} onChange={e => setEditingItem({...editingItem, size: e.target.value})}/></div>
              <div className="space-y-2"><label className="text-emerald-500 font-black ml-1">CURRENT STOCK</label><input type="number" className="w-full bg-black border border-emerald-500/20 p-5 rounded-2xl outline-none text-emerald-400 font-black text-lg" value={editingItem.quantity} onChange={e => setEditingItem({...editingItem, quantity: e.target.value})}/></div>
              <div className="space-y-2"><label className="text-red-500 font-black ml-1">RESTOCK AT</label><input type="number" className="w-full bg-black border border-red-500/20 p-5 rounded-2xl outline-none text-red-400 font-black text-lg" value={editingItem.threshold} onChange={e => setEditingItem({...editingItem, threshold: e.target.value})}/></div>
            </div>
      ...
      {isEditCrewOpen && editingCrew && (
        <div className="fixed inset-0 z-[1000] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl animate-in zoom-in">
          <div className="bg-zinc-900 border border-white/10 rounded-[48px] w-full max-w-4xl max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="flex justify-between items-center border-b border-white/5 p-10 shrink-0"><h2 className="text-3xl font-black italic uppercase text-orange-500">{editingCrew.full_name}</h2><button onClick={() => setIsEditCrewOpen(false)} className="p-3 bg-white/5 rounded-full hover:bg-orange-600 transition-all"><X size={32}/></button></div>
            <div className="overflow-y-auto p-10 space-y-12 flex-1 no-scrollbar">
               <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-white">
                    ...
                    <div className="space-y-2"><label className="text-zinc-600 font-bold uppercase text-[9px]">Boiler Suit Size</label><input className="w-full bg-black border border-white/10 p-5 rounded-2xl" value={editingCrew.suit_size || '-'} onChange={e => setEditingCrew({...editingCrew, suit_size: e.target.value})}/></div>
                    <div className="space-y-2"><label className="text-zinc-600 font-bold uppercase text-[9px]">Safety Boot Size</label><input className="w-full bg-black border border-white/10 p-5 rounded-2xl" value={editingCrew.boot_size || '-'} onChange={e => setEditingCrew({...editingCrew, boot_size: e.target.value})}/></div>
                  </div>
                  <div className="flex gap-4 pt-4">
                     <button onClick={handleUpdateCrew} className="flex-1 py-5 bg-orange-600 rounded-3xl font-black uppercase text-xs shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all"><Save size={18}/> Update Member Information</button>
