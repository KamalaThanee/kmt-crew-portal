...
import { 
  ...
  Loader2, Upload, RefreshCw, X, Save, AlertTriangle, XCircle, ChevronRight, ChevronDown, Plus, Box
} from 'lucide-react'
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          <div className="w-full lg:w-64 space-y-3 shrink-0">
            {['crews', 'system'].map(t => (
              <button key={t} onClick={() => { setActiveTab(t); setSearchTerm(''); }} className={`w-full flex items-center gap-4 p-5 rounded-[24px] transition-all border ${activeTab === t ? 'bg-orange-600 border-orange-400 text-white shadow-lg shadow-blue-600/20' : 'bg-zinc-900 text-zinc-500 hover:text-orange-400'}`}>
      ...
      {isEditCrewOpen && editingCrew && (
        <div className="fixed inset-0 z-[1000] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl animate-in zoom-in">
          <div className="bg-zinc-900 border border-white/10 rounded-[48px] w-full max-w-4xl max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="flex justify-between items-center border-b border-white/5 p-10 shrink-0 text-orange-500"><h2 className="text-3xl font-black italic uppercase tracking-tighter">{editingCrew.full_name}</h2><button onClick={() => setIsEditCrewOpen(false)} className="p-3 bg-white/5 rounded-full hover:bg-orange-600"><X size={32}/></button></div>
            <div className="overflow-y-auto p-10 space-y-12 flex-1 no-scrollbar">
               <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-white uppercase text-[10px]">
                    ...
                    <div className="space-y-2"><label className="text-zinc-600 font-bold">Boiler Suit Size</label><select className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold uppercase text-xs" value={editingCrew.suit_size || ''} onChange={e => setEditingCrew({...editingCrew, suit_size: e.target.value})}><option value="">-- Size --</option>{suitSizes.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div className="space-y-2"><label className="text-zinc-600 font-bold">Safety Boot Size</label><select className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none text-white font-bold uppercase text-xs" value={editingCrew.boot_size || ''} onChange={e => setEditingCrew({...editingCrew, boot_size: e.target.value})}><option value="">-- Size --</option>{bootSizes.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                  </div>
