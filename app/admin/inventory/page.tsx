...
import { Package, Search, AlertTriangle, Box, FileText, Download, X } from 'lucide-react'
...
export default function InventoryPage() {
  ...
  useEffect(() => {
    // 🎯 เช็ค URL ถ้ามี ?filter=low ให้เปิด filter อัตโนมัติ
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-32 pt-2 md:pt-6">
      <div className="mb-8 flex justify-between items-center">
        ...
        {/* 🎯 ปุ่ม Export Excel ใน Navbar หน้า Inventory */}
      ...
      <div className="space-y-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <button onClick={() => setShowLowStock(!showLowStock)} className={`py-4 rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 border transition-all ${showLowStock ? 'bg-red-500 border-red-500 text-white' : 'bg-white/5 border-white/5 text-slate-400'}`}><AlertTriangle size={16} /> {showLowStock ? 'Low Stock Active' : 'Filter Low Stock'}</button>
        </div>
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar py-2">
          {categoryConfig.map(cat => {
            const isActive = selectedCats.includes(cat.name);
            return (<button key={cat.name} onClick={() => toggleCat(cat.name)} className={`px-5 py-3 rounded-2xl border transition-all font-black text-[10px] uppercase whitespace-nowrap ${isActive ? 'bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-600/30' : 'bg-slate-900 border-white/5 text-slate-500 hover:border-white/10'}`}>{cat.label}</button>)
          })}
        </div>
      </div>
