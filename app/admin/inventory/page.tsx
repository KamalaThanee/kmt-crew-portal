...
function InventoryContent() {
  ...
  const fetchData = async () => {
    // 🎯 เรียงลำดับ Inventory ตาม Category และ ID Code ตั้งแต่ต้นทาง
    const { data: inv } = await supabase.from('ppe_inventory').select('*')
      .order('category', { ascending: true })
      .order('item_id_code', { ascending: true })
  ...
  useEffect(() => {
    ...
    // 🎯 ตรวจสอบคำสั่งเปิด Modal รับของจากหน้า Dashboard
  ...
  const handleRestockSubmit = async () => {
    ...
    if (validEntries.length === 0 || !doFile) return toast.error('Required fields missing')
    ...
    try {
      ...
      await supabase.storage.from('do-files').upload(fileName, fileToUpload)
    ...
    <div className="p-4 md:p-12 max-w-[1600px] mx-auto pb-32 pt-28 font-sans text-white uppercase font-bold text-[10px]">
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
           <button onClick={() => { setRestockView('entry'); setIsRestockModalOpen(true); }} className="px-8 py-4 bg-emerald-600/10 border border-emerald-500/30 text-emerald-500 rounded-[20px] font-black text-xs flex items-center gap-3 active:scale-95 transition-all"><Archive size={18}/> Receive Stock</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-zinc-900/50 p-6 rounded-[40px] border border-white/5 mb-10">
        <div className="relative md:col-span-2"><Search className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600" size={20}/><input type="text" placeholder="Search..." className="w-full bg-black/60 border border-white/10 rounded-[24px] py-4 pl-14 text-white outline-none focus:border-orange-500 transition-all text-sm font-bold" onChange={(e) => setSearchTerm(e.target.value)} /></div>
      ...
      {isItemModalOpen && editingItem && (
        <div className="fixed inset-0 z-[2000] bg-black/98 flex items-center justify-center p-6 backdrop-blur-xl">
          <div className="bg-zinc-900 border border-orange-500/30 rounded-[56px] w-full max-w-xl p-12 space-y-8 shadow-2xl overflow-y-auto max-h-[92vh] no-scrollbar">
            <div className="flex justify-between items-center border-b border-white/5 pb-8 text-orange-500"><h2 className="text-3xl font-black italic uppercase tracking-tighter">Edit Item</h2><button onClick={() => setIsItemModalOpen(false)} className="p-4 bg-white/5 rounded-full hover:bg-red-500 text-white transition-all text-white"><X size={24}/></button></div>
                  ...
                  <div className="space-y-6">
                    ...
                    {restockEntries.map((entry, index) => (
                      <div key={entry.id} className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center bg-black/50 p-6 rounded-[32px] border border-white/5 group hover:border-emerald-500/30 transition-all">
                        <div className="md:col-span-8"><select value={entry.inventory_id} onChange={(e) => updateRow(entry.id, 'inventory_id', e.target.value)} className="w-full bg-transparent text-base text-white font-black outline-none cursor-pointer p-2 uppercase"><option value="" className="bg-zinc-900">-- Select Product To Add --</option>{inventory.map(i => <option key={i.id} value={i.id} className="bg-zinc-900">{i.item_id_code ? `[${i.item_id_code}] ` : ''}{i.item_name} {i.color ? ` - ${i.color}` : ''} {i.size ? ` (${i.size})` : ''}</option>)}</select></div>
                        <div className="md:col-span-3"><input type="number" min="1" placeholder="QTY" value={entry.qty} onChange={(e) => updateRow(entry.id, 'qty', e.target.value)} className="w-full bg-zinc-950 border-2 border-emerald-500/20 p-5 rounded-2xl text-center font-black text-emerald-400 outline-none focus:border-emerald-500 text-2xl shadow-inner" /></div>
                ...
                <div className="space-y-6 animate-in fade-in max-w-4xl mx-auto pb-20">
                  ...
                  {history.map(h => (
                    <div key={h.id} className="bg-black/40 border border-white/5 p-8 rounded-[40px] flex justify-between items-center hover:bg-white/5 transition-all group shadow-xl">
                      ...
                      <div className="text-right"><p className="text-emerald-500 font-black text-2xl">+{h.quantity_added}</p><p className="text-[10px] text-zinc-700 font-black uppercase tracking-tighter mt-1">Stock Added</p></div>
                    </div>
                  ))}
                </div>
              )}
