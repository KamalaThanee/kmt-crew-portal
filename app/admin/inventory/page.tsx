...
function InventoryContent() {
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-orange-500 font-black animate-pulse uppercase">Accessing Inventory...</div>
  ...
  return (
    <div className="p-4 md:p-12 max-w-[1600px] mx-auto pb-32 pt-28 font-sans text-white uppercase font-bold text-[10px]">
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div><h1 className="text-4xl md:text-5xl font-black italic flex items-center gap-4 tracking-tighter"><Package className="text-blue-500" size={40}/> Inventory</h1></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-zinc-900/50 p-6 rounded-[40px] border border-white/5 mb-10 shadow-inner">
        ...
        <select className="bg-black/60 border border-white/10 rounded-[24px] py-4 px-6 text-white outline-none focus:border-orange-500 text-xs font-black appearance-none text-center" value={selectedCat} onChange={(e) => setSelectedCat(e.target.value)}>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select>
                      ...
                      {items.map(item => {
                        ...
                        return (
                          <tr key={item.id} className="bg-white/5 hover:bg-orange-600/10 transition-all rounded-2xl group">
                            ...
                            <td className="p-6 text-center pr-8 rounded-r-[24px]"><button onClick={() => { setEditingItem(item); setIsItemModalOpen(true); }} className="p-3 bg-black/50 border border-white/10 rounded-xl text-orange-400 hover:bg-orange-600 transition-all"><Edit size={16}/></button></td>
                          </tr>
                        )
                      })}
                    </tbody>
        ...
        <div className="fixed inset-0 z-[2000] bg-black/98 flex items-center justify-center p-4 md:p-6 backdrop-blur-3xl animate-in zoom-in duration-300">
          <div className="bg-zinc-900 border border-emerald-500/30 rounded-[56px] w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-center border-b border-white/5 p-10 shrink-0">
               <div><h2 className="text-4xl font-black italic uppercase tracking-tighter text-emerald-500 flex items-center gap-4"><Archive size={36}/> Receive Shipment</h2><div className="flex gap-2 mt-6 bg-black/60 p-1.5 rounded-[20px] w-fit"><button onClick={() => setRestockView('entry')} className={`px-10 py-3 rounded-2xl text-xs font-black uppercase transition-all ${restockView === 'entry' ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-600'}`}>New Entry</button><button onClick={() => setRestockView('history')} className={`px-10 py-3 rounded-2xl text-xs font-black uppercase transition-all ${restockView === 'history' ? 'bg-emerald-600 text-white' : 'text-zinc-600'}`}>History</button></div></div>
