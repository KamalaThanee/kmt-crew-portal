...
import { FileBadge, Users, User, AlertTriangle, ChevronRight, CheckCircle2, ShieldCheck, Package, RefreshCw, Clock, Archive, Activity } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  async function fetchAdminStats(u: any) {
    ...
    setVessel({ pending: pending || 0, lowStock: inventory?.filter(i => (i.quantity||0) <= (i.threshold||0)).length || 0, totalItems: inventory?.reduce((a, b) => a + (b.quantity || 0), 0) || 0, compliance: totalFleetReq > 0 ? Math.round((totalFleetOk/totalFleetReq)*100) : 0, vesselExpired: allCerts?.filter(c => new Date(c.expiry_date) < new Date() && c.expiry_date !== '2099-12-31').length || 0 })
    setLoading(false)
  }

  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-950 text-blue-500 font-black animate-pulse text-xs uppercase tracking-widest">Admin Hub Loading...</div>

  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-32 pt-20 font-sans text-white uppercase font-bold text-[10px]">
      <div className="mb-10 flex justify-between items-center">
        <div><h1 className="text-3xl font-black italic">Command Center</h1><p className="text-slate-500 mt-2 tracking-widest">Personal & Fleet Control</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 space-y-6">
           <h2 className="text-blue-500 tracking-widest flex items-center gap-2 mb-2"><User size={14}/> My Personal</h2>
           <Link href="/certificates" className="block bg-slate-900 border border-blue-500/20 p-6 rounded-[32px] shadow-2xl hover:border-blue-500 transition-all">
              <div className="flex justify-between items-center mb-4">
                 ...
                 <div className="text-right"><p className="text-blue-400 text-lg">{personal.okCount}/{personal.reqCount}</p><p className="text-[7px] text-slate-500">Certs</p></div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                 <div className="bg-red-500/10 p-2 rounded-xl text-center"><p className="text-red-500 text-xs">{personal.expired}</p><p className="text-[6px] text-red-500/50 uppercase">Expired</p></div>
                 <div className="bg-amber-500/10 p-2 rounded-xl text-center"><p className="text-amber-500 text-xs">{personal.warning}</p><p className="text-[6px] text-amber-500/50 uppercase">90 Days</p></div>
              </div>
           </Link>
        ...
        <div className="lg:col-span-3 space-y-6">
           <h2 className="text-purple-500 tracking-widest flex items-center gap-2 mb-2"><ShieldCheck size={14}/> My Vessel</h2>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Link href="/admin/settings?tab=crews" className="col-span-2 md:col-span-4 bg-slate-900 border border-purple-500/20 p-8 rounded-[40px] flex items-center justify-between shadow-2xl hover:border-purple-500 transition-all">
                 <div className="flex items-center gap-6"><div className="w-16 h-16 bg-purple-500/10 rounded-3xl flex items-center justify-center text-purple-500 font-black text-xl border border-purple-500/20">{vessel.compliance}%</div><div><p className="text-lg font-black">Fleet Readiness</p><p className="text-slate-500 uppercase text-[8px]">View Full Compliance Report</p></div></div>
                 <div className="text-right"><p className="text-red-500 text-2xl font-black">{vessel.vesselExpired}</p><p className="text-slate-600 uppercase text-[8px]">Expired Certs</p></div>
              </Link>
           </div>
        </div>
      </div>
