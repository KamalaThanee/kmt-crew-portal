...
export default function AdminDashboard() {
  ...
  async function fetchAdminStats(u: any) {
    ...
    crews?.forEach(crew => {
      const userPosNorm = normalize(crew.position);
      ...
      req.forEach(r => {
        totalRequired++;
        const c = crewC.find(mc => normalize(mc.cert_name) === normalize(r.cert_name))
        if (c && (new Date(c.expiry_date) >= new Date() || c.expiry_date === '2099-12-31')) totalOk++;
      })
    ...
    setStats({ 
      ...
      totalInventory: inventory?.reduce((a, b) => a + (b.quantity||0), 0) || 0, 
      lastRestock, fleetCompliance: compliance, fleetExpired,
      topExpired: crewAlerts.sort((a,b) => b.expired - a.expired).slice(0, 3)
    })
    setLoading(false)
  }

  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-950 text-blue-500 font-black animate-pulse uppercase text-xs tracking-widest">Command Center Loading...</div>

  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-32 pt-20 font-sans text-white uppercase font-bold text-[10px]">
      <div className="mb-10 flex justify-between items-center">
        <div><h1 className="text-3xl font-black italic leading-none">Command Center</h1><p className="text-slate-500 tracking-[0.2em] mt-2">Vessel Oversight & Personal Compliance</p></div>
              ...
              <div className="col-span-2 md:col-span-4 bg-slate-900 border border-white/5 p-6 rounded-[40px] shadow-2xl flex flex-col md:flex-row gap-8">
                 <Link href="/admin/settings?tab=crews" className="flex-1 flex items-center gap-6 group">
                    ...
                    <div>
                       ...
                       <p className="mt-3 text-red-500 font-black flex items-center gap-1"><AlertTriangle size={14}/> {stats.fleetExpired} Total Expired</p>
                    </div>
                 </Link>
                 <div className="flex-1 bg-black/30 rounded-3xl p-4 border border-white/5 space-y-2">
                    ...
                    {stats.topExpired.length === 0 ? (
                       <p className="text-emerald-500 py-4 text-center">All clear!</p>
                    ) : (
