...
export default function AdminDashboard() {
  ...
  async function fetchStats(u: any) {
    ...
    // --- 🎯 Personal Stats Logic (Same as Cert Page) ---
    ...
    let required = matrix?.filter(row => {
      const colKey = Object.keys(row).find(k => normalize(k) === 'position');
      return colKey && normalize(row[colKey]) === userPosNorm && row.requirement_type === 'P';
    ...
    setPersonal({ progress: required.length > 0 ? Math.round((okCount / required.length) * 100) : 0, okCount, reqCount: required.length, expired, warning, missing: required.length - okCount, suit, boot, lastStatus: myReqs?.[0]?.status || 'None' });

    // --- Vessel Stats ---
    const lowStockCount = inventory?.filter(i => (i.quantity||0) <= (i.threshold||0)).length || 0
    setVessel({ pending: pending || 0, lowStock: lowStockCount, totalItems: inventory?.reduce((a, b) => a + (b.quantity||0), 0) || 0, compliance: 85, vesselExpired: 0, vesselWarning: 0 });
    setLoading(false)
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-32 pt-20 font-sans text-white uppercase font-bold text-[10px]">
      <div className="mb-10 flex justify-between items-center">
        <div><h1 className="text-3xl font-black italic">Command Center</h1><p className="text-slate-500 mt-2">Personal & Fleet Oversight</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 space-y-6">
           ...
           <div className="bg-slate-900 border border-white/5 p-6 rounded-[32px] space-y-4 shadow-xl">
              <div className="flex justify-between items-center"><p className="text-slate-500 uppercase">PPE Quota</p><span className="text-blue-400">{personal.lastStatus}</span></div>
              <div className="flex gap-4">
                 <div className="flex-1 text-center bg-black/30 p-2 rounded-xl border border-white/5"><p className="text-[7px] text-slate-600">SUIT</p><p className="font-black text-sm">{personal.suit}/2</p></div>
                 <div className="flex-1 text-center bg-black/30 p-2 rounded-xl border border-white/5"><p className="text-[7px] text-slate-600">BOOTS</p><p className="font-black text-sm">{personal.boot}/1</p></div>
              </div>
        ...
        <div className="lg:col-span-3 space-y-6">
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Link href="/admin/approvals" className="bg-slate-900 border border-amber-500/30 p-5 rounded-[32px] flex flex-col justify-between h-40 shadow-lg hover:border-amber-500 transition-all"><Clock className="text-amber-500" size={20}/><p className="text-2xl font-black">{vessel.pending}</p><p className="text-amber-500 uppercase text-[9px]">Pending</p></Link>
              <Link href="/admin/inventory?filter=low" className="bg-slate-900 border border-red-500/30 p-5 rounded-[32px] flex flex-col justify-between h-40 shadow-lg hover:border-red-500 transition-all"><AlertTriangle className="text-red-500" size={20}/><p className="text-2xl font-black">{vessel.lowStock}</p><p className="text-red-500 uppercase text-[9px]">Low Stock</p></Link>
              <Link href="/admin/inventory" className="bg-slate-900 border border-blue-500/30 p-5 rounded-[32px] flex flex-col justify-between h-40 shadow-lg hover:border-blue-500 transition-all"><Package className="text-blue-500" size={20}/><p className="text-2xl font-black">{vessel.totalItems}</p><p className="text-blue-500 uppercase text-[9px]">Inventory</p></Link>
              <Link href="/admin/restock" className="bg-slate-900 border border-emerald-500/30 p-5 rounded-[32px] flex flex-col justify-between h-40 shadow-lg hover:border-emerald-500 transition-all"><Archive className="text-emerald-500" size={20}/><p className="text-lg font-black truncate">RECEIVE</p><p className="text-emerald-500 uppercase text-[9px]">Control</p></Link>
           </div>
        </div>
      </div>
    </div>
