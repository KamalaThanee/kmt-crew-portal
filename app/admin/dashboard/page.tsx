...
import { AlertCircle, CheckSquare, PackagePlus, ArrowLeft, Clock } from "lucide-react";
import Link from "next/link";

// 1. กำหนดโครงสร้างข้อมูลให้ TypeScript รู้จัก
...
export default function AdminDashboard() {
  // 2. ระบุ Type <DashboardStats> ให้กับ State
  ...
  useEffect(() => {
    async function loadStats() {
      ...
      // 3. ใส่ Type Casting (as any[]) เพื่อความชัวร์ตอน Build
      setStats({ 
        pending: count || 0, 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2rem]">
            ...
            <div className="mt-4 space-y-2">
              {stats.lowStock.slice(0, 2).map((item, idx) => (
                <div key={idx} className="text-[10px] bg-rose-500/10 p-2 rounded-lg border border-rose-500/20">
