...
function CertificatesContent() {
  ...
  const certList = useMemo(() => {
    ...
    // 🎯 แก้ไข Logic: กรองแถวที่คอลัมน์ 'position' ตรงกับตำแหน่ง User
    let required = matrix.filter(row => normalize(row.position) === userPosNorm && row.requirement_type === 'P')
      .map(m => ({ ...m, is_mandatory: true }))

    const optionals = matrix.filter(row => normalize(row.position) === userPosNorm && row.requirement_type === 'O')
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto pb-32 pt-20 font-sans text-white uppercase font-bold text-[10px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
         ...
         <button onClick={() => setFilter('ok')} className={`p-4 rounded-2xl border transition-all flex flex-col items-center justify-center gap-1 ${filter === 'ok' ? 'bg-emerald-500 border-emerald-400 text-white' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500'}`}>
            <span className="text-2xl font-black">{stats.ok}</span><span>READY</span>
         </button>
         <button onClick={() => setFilter('warning')} className={`p-4 rounded-2xl border transition-all flex flex-col items-center justify-center gap-1 ${filter === 'warning' ? 'bg-amber-500 border-amber-400 text-white' : 'bg-amber-500/10 border-amber-500/20 text-amber-500'}`}>
            <span className="text-2xl font-black">{stats.warning}</span><span>90 DAYS</span>
         </button>
         <button onClick={() => setFilter('expired')} className={`p-4 rounded-2xl border transition-all flex flex-col items-center justify-center gap-1 ${filter === 'expired' ? 'bg-red-500 border-red-400 text-white' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
            <span className="text-2xl font-black">{stats.expired}</span><span>EXPIRED</span>
         </button>
         <button onClick={() => setFilter('missing')} className={`p-4 rounded-2xl border transition-all flex flex-col items-center justify-center gap-1 ${filter === 'missing' ? 'bg-slate-600 border-slate-500 text-white' : 'bg-slate-800 border-white/5 text-slate-500'}`}>
