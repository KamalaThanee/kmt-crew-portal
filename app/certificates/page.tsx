...
function CertificatesContent() {
  ...
  const certStatusList = useMemo(() => {
    ...
    // 🎯 Logic พิเศษ: ค้นหาคอลัมน์ที่ตรงกับตำแหน่งลูกเรือ (ไม่สนเว้นวรรค)
    const posKey = user.position.trim();
    const required = matrix.filter(row => row[posKey] === 'P')
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-950 text-blue-500 font-black animate-pulse text-xs">VAULT LOADING...</div>
