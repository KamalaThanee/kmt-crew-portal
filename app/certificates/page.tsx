...
function CertificatesContent() {
  ...
  const [filterPos, setFilterPos] = useState('All') // 🎯 กู้คืน
  const [filterSpecificCert, setFilterSpecificCert] = useState('All') // 🎯 กู้คืน
  ...
  // 🎯 ปรับ Logic ให้กรองอย่างถูกต้องและนับสถิติจากคนที่ผ่าน Filter ແล้ວ
  ...
  // สถิติข้างบนคำนวณจากคนที่รอดมาจาก Filter เบื้องต้นแล้ว
  const crewSummary = useMemo(() => {
    return {
      total: enhancedCrews.length,
      ok: enhancedCrews.filter(c => c.certData.progress === 100 && c.certData.expired === 0).length,
  ...
  // ลิสต์คนสุดท้ายที่เอาไป Render หน้าเว็บ (บวก Filter 5 สีเข้าไปด้วย)
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-32 pt-28 font-sans text-white uppercase font-bold text-[10px]">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        {isAdmin && (
          <div className="flex bg-zinc-900 p-1.5 rounded-2xl border border-white/5 w-fit shadow-2xl">
            <button onClick={() => setActiveTab('personal')} className={`px-6 py-3 rounded-xl transition-all ${activeTab === 'personal' ? 'bg-orange-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}>My Certs</button>
            <button onClick={() => setActiveTab('crew')} className={`px-6 py-3 rounded-xl transition-all ${activeTab === 'crew' ? 'bg-orange-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}>Crew Certificates</button>
            <button onClick={() => setActiveTab('ship')} className={`px-6 py-3 rounded-xl transition-all ${activeTab === 'ship' ? 'bg-orange-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}>Ship Certs</button>
          </div>
        )}
      </div>
        ...
        <div className="space-y-8 animate-in fade-in">
           ...
           <div className="space-y-3">
              {filteredMyList.map((item, idx) => (
                <div key={idx} className={`bg-zinc-900 border ${item.status === 'missing' ? 'border-red-500/20' : item.status === 'optional' ? 'border-white/5 opacity-50' : 'border-white/10'} rounded-[24px] p-5 flex items-center justify-between hover:border-orange-500/30 transition-all`}>
                  ...
                  <div className="flex gap-2">
                     {item.uploaded && <a href={item.uploaded.file_url} target="_blank" rel="noopener noreferrer" className="p-3 bg-white/5 rounded-xl text-zinc-400 hover:text-white transition-all"><Eye size={18}/></a>}
                     <button onClick={() => router.push(`/certificates/upload?cert=${encodeURIComponent(item.cert_name)}`)} className="px-5 py-3 bg-orange-600 rounded-xl text-[9px] font-black uppercase shadow-lg active:scale-95">Update</button>
                  </div>
                </div>
              ))}
           </div>
      ...
      {activeTab === 'crew' && (
        <div className="space-y-8 animate-in fade-in">
           ...
           {/* 🎯 6-Tile Filters (คำนวณใหม่ตามยอดที่ผ่าน Dropdowns แล้ว) */}
           <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {[
                ...
                { id: 'warning', label: 'ใกล้หมด', val: crewSummary.warning, color: 'border-orange-500', icon: <Clock size={16}/> },
                ...
                <button key={tile.id} onClick={() => setFilterMode(tile.id)} className={`bg-zinc-900 border-t-4 ${tile.color} p-5 rounded-3xl flex flex-col items-center justify-center transition-all active:scale-95 shadow-xl ${filterMode === tile.id ? 'opacity-100 ring-2 ring-white/20' : 'opacity-40 hover:opacity-100'}`}>
                     ...
                     <button onClick={() => setExpandedCrews(prev => prev.includes(crew.id) ? prev.filter(id => id !== crew.id) : [...prev, crew.id])} className="w-full p-6 flex flex-col outline-none">
                        <div className="w-full flex justify-between items-center mb-3">
                           <div className="flex items-center gap-4">
                             ...
                             <div className="text-left"><p className="font-black text-sm text-white uppercase">{crew.full_name}</p><p className="text-[9px] text-zinc-500 tracking-widest mt-1">{crew.position}</p></div>
                           </div>
                           <div className="flex items-center gap-4">
                       ...
                       <div className="p-6 pt-0 border-t border-white/5 bg-black/20 animate-in slide-in-from-top-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                             {crew.certData.list.map((c: any, i: number) => (
                               <div key={i} className={`flex items-center justify-between p-4 rounded-2xl border-l-4 bg-zinc-900/50 border-y border-r border-y-white/5 border-r-white/5 ${c.status === 'ok' ? 'border-l-emerald-500' : c.status === 'expired' ? 'border-l-red-500' : c.status === 'warning' ? 'border-l-amber-500' : 'border-l-slate-600'}`}>
                                  <div className="flex items-center gap-3">
                                     <div className={`p-2 rounded-xl ${c.status === 'ok' ? 'bg-emerald-500/10 text-emerald-500' : c.status === 'warning' ? 'bg-amber-500/10 text-amber-500' : c.status === 'expired' ? 'bg-red-500/10 text-red-500' : 'bg-slate-800 text-slate-500'}`}>
                                        {c.status === 'missing' ? <AlertTriangle size={16}/> : c.status === 'expired' ? <XCircle size={16}/> : c.status === 'optional' ? <Clock size={16}/> : <CheckCircle2 size={16}/>}
                                     </div>
                                     <div>
                                        <p className="text-white text-[10px] font-black leading-tight uppercase flex items-center gap-2">{c.cert_name} <span className="text-[7px] text-zinc-600 font-bold">{c.is_mandatory ? '(M)' : '(O)'}</span></p>
                                        <p className={`text-[8px] mt-1 font-bold uppercase ${c.status === 'ok' ? 'text-emerald-500' : c.status === 'expired' ? 'text-red-500' : 'text-zinc-500'}`}>
                                  ...
                                  <div className="flex gap-2 shrink-0">
                                     {c.uploaded ? (
                                       <>
                                         <a href={c.uploaded.file_url} target="_blank" rel="noopener noreferrer" className="p-2 bg-white/5 rounded-lg text-orange-500"><Eye size={16}/></a>
                                         <button onClick={() => router.push(`/certificates/upload?cert=${encodeURIComponent(c.cert_name)}&crewId=${crew.id}`)} className="p-2 bg-orange-600/10 text-orange-500 rounded-lg"><RefreshCcw size={16}/></button>
                                       </>
                                     ) : (
                                       <button onClick={() => router.push(`/certificates/upload?cert=${encodeURIComponent(c.cert_name)}&crewId=${crew.id}`)} className="px-4 py-2 bg-orange-600 text-white rounded-lg shadow-lg active:scale-90 text-[9px] font-black">Upload</button>
                                     )}
                                  </div>
                               </div>
                             ))}
                          </div>
                          <button onClick={() => router.push(`/admin/settings?tab=crews&id=${crew.id}`)} className="w-full py-4 mt-6 bg-zinc-800 border border-white/5 rounded-2xl text-[9px] font-black uppercase text-zinc-400 hover:text-white transition-all flex items-center justify-center gap-2">Edit Crew Profile (Sizes & PIN) <ChevronRight size={14}/></button>
                       </div>
                     )}
                  </div>
                )
              })}
              {finalDisplayCrews.length === 0 && <div className="text-center py-20 text-zinc-600 font-black tracking-widest italic">NO RECORDS MATCH YOUR FILTERS</div>}
           </div>
        </div>
      )}
