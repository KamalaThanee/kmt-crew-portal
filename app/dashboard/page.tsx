      ...
      <div className="space-y-6">
        <Link href="/certificates" className="block bg-slate-900 border border-blue-500/20 p-8 rounded-[40px] shadow-2xl flex items-center justify-between hover:border-blue-500 transition-all">
          <div className="flex items-center gap-8">
            ...
            <div><h3 className="text-lg font-black italic">My Readiness</h3><p className="text-slate-500 mt-1">Status: <span className={stats.expired > 0 ? 'text-red-500' : 'text-emerald-500'}>{stats.expired > 0 ? 'Expired Certs' : 'Ready'}</span></p></div>
          </div>
          <ChevronRight className="text-slate-700"/>
        </Link>

        <div className="grid grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-white/5 p-8 rounded-[40px] flex flex-col justify-between h-44 shadow-xl">
             ...
             <Link href="/ppe" className="text-blue-400 flex items-center gap-1">Request New <ChevronRight size={12}/></Link>
          </div>
