...
function PPEContent() {
  ...
  useEffect(() => {
    async function fetchQuotas() {
      const target = JSON.parse(localStorage.getItem('kmt_target_crew') || 'null') || user;
      if (!target?.id) return;
      const currentYear = new Date().getFullYear()
      const { data: reqs } = await supabase.from('ppe_requests').select('items').eq('crew_id', target.id).neq('status', 'rejected').gte('created_at', `${currentYear}-01-01`)
      // Logic for quota display could be added here if needed in header
    }
    fetchQuotas()
  }, [user])
  ...
  const groupedInventory = useMemo(() => {
    ...
    filtered.forEach(item => {
      ...
      if (isAdmin) {
        ...
        if (isSuit) {
          if (String(item.size) === String(target?.suit_size) && String(item.color) === String(target?.suit_color)) groups[item.item_name].push(item);
        } else if (isBoot) {
          if (String(item.size) === String(target?.boot_size)) groups[item.item_name].push(item);
        } else {
          groups[item.item_name].push(item);
        }
      }
    ...
    <div className="min-h-screen bg-black text-white pb-32 pt-20 px-4 md:px-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 border-b border-white/5 pb-6">
           <div>
              <h1 className="text-2xl md:text-4xl font-black italic uppercase text-white tracking-tighter leading-none">Request PPE</h1>
              <p className="text-orange-500 text-[10px] font-bold uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
                {isAdmin ? <ShieldAlert size={12}/> : <User size={12}/>}
                {isAdmin ? 'Storekeeper Access' : `Personnel: ${user.full_name}`}
              </p>
           </div>
                   ...
                   <button onClick={() => setExpandedItemName(isExpanded ? null : group.name)} className="w-full p-6 flex items-center justify-between outline-none">
                      <div className="flex items-center gap-4">
                         ...
                         <div className="text-left">
                            ...
                            <p className="text-[9px] text-zinc-500 font-bold uppercase mt-1">{group.variants.length} Options</p>
                         </div>
                      </div>
...
export default function PPEPage() { return ( <Suspense fallback={<div>Loading Storefront...</div>}><PPEContent /></Suspense> ) }
