...
export default function PPEPage() {
  ...
  const handleFileUpload = async (event, type) => {
    try {
      ...
      // 1. Upload file to Supabase Storage (Bucket: ppe_assets)
      ...
      const newCharts = { ...sizeCharts, [type]: publicUrl }
      setSizeCharts(newCharts)
      localStorage.setItem('kmt_size_charts', JSON.stringify(newCharts))
  ...
  const handleCheckout = async () => {
    ...
    if (!error) {
      alert('บันทึกคำขอเบิกเรียบร้อยแล้ว'); setCart([]); setShowCart(false); window.location.reload(); 
    }
    setLoading(false)
  }

  if (!mounted || !user) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">System Booting...</div>
              ...
              {isCatOpen && (
                <div className="space-y-2 pt-1 animate-in slide-in-from-top-2 duration-300">
                  {catItems.map((group) => {
                    ...
                    const isSafetyBoot = lowerName.includes('safety boot')
                      ...
                      <div key={group.name} className="bg-white/5 border border-white/5 rounded-2xl overflow-hidden">
                        <button onClick={() => setExpandedItem(isItemOpen ? null : group.name)} className="w-full p-4 flex items-center justify-between">
                          <div className="flex flex-col items-start text-left">
                            ...
                            {limit.reached && <span className="text-[8px] text-red-500 font-black uppercase mt-1 flex items-center gap-1"><ShieldAlert size={10}/> {limit.msg}</span>}
                          </div>
                          <div className="flex items-center gap-3">
                              ...
                              return (
                                <div key={vIdx} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
                                  <div className="flex flex-col">
                                    <span className="text-[11px] font-bold">{vColor && vSize !== 'Standard' ? `${vColor} - ${vSize}` : (vColor || vSize)}</span>
                                    {hasFullAccess ? (
          ...
          <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-white/10 overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              <div>
                ...
                <p className="text-[9px] text-slate-500 font-bold uppercase">อัปโหลดตารางไซส์</p>
              </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-500 uppercase flex justify-between items-center">
                  Boiler Suit Size Chart
                  {sizeCharts.suit && <span className="text-emerald-500 flex items-center gap-1"><CheckCircle2 size={10}/> Uploaded</span>}
                </label>
                <div className="relative group">
                  <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, 'suit')} className="hidden" id="suit-upload" disabled={uploading.suit}/>
                  <label htmlFor="suit-upload" className="w-full flex flex-col items-center justify-center py-8 border-2 border-dashed border-white/10 rounded-2xl hover:border-blue-500/50 transition-all cursor-pointer bg-black/20">
                    {uploading.suit ? <Loader2 className="animate-spin text-blue-500"/> : <Upload className="text-slate-400 group-hover:text-blue-500 transition-colors mb-2"/>}
                    <span className="text-[10px] font-bold text-slate-500">{uploading.suit ? 'Uploading...' : 'Click to Upload Image'}</span>
                  </label>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-500 uppercase flex justify-between items-center">
                  Safety Boots Size Chart
                  {sizeCharts.boot && <span className="text-emerald-500 flex items-center gap-1"><CheckCircle2 size={10}/> Uploaded</span>}
                </label>
                <div className="relative group">
                  <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, 'boot')} className="hidden" id="boot-upload" disabled={uploading.boot}/>
                  <label htmlFor="boot-upload" className="w-full flex flex-col items-center justify-center py-8 border-2 border-dashed border-white/10 rounded-2xl hover:border-blue-500/50 transition-all cursor-pointer bg-black/20">
                    {uploading.boot ? <Loader2 className="animate-spin text-blue-500"/> : <Upload className="text-slate-400 group-hover:text-blue-500 transition-colors mb-2"/>}
                    <span className="text-[10px] font-bold text-slate-500">{uploading.boot ? 'Uploading...' : 'Click to Upload Image'}</span>
                  </label>
                </div>
              </div>

              <button onClick={() => setShowSettings(false)} className="w-full py-4 bg-white/5 border border-white/10 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all hover:bg-white/10">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showCart && (
        <div className="fixed inset-0 bg-slate-950 z-[60] flex flex-col animate-in slide-in-from-bottom duration-300">
          <div className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-900">
            <h3 className="text-sm font-black uppercase text-blue-500">Cart Review</h3>
            <button onClick={() => setShowCart(false)} className="p-2 bg-white/10 rounded-full"><X size={20}/></button>
          </div>
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-20"><Package size={64}/><p className="text-xs font-bold mt-4 uppercase">Cart is empty</p></div>
            ) : cart.map((item) => (
          ...
          <div className="p-8 border-t border-white/10 bg-slate-900/50 backdrop-blur-md">
            <button disabled={cart.length === 0 || loading} onClick={handleCheckout} className="w-full py-5 bg-blue-600 rounded-2xl font-black uppercase tracking-widest shadow-2xl shadow-blue-500/30 active:scale-95 transition-all">
